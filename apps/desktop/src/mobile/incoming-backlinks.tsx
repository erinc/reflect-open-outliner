 ...
 * chevrons instead of hover-revealed ones. The header collapses the entire
 * list of sources and linking lines, shared live across every mounted surface
 * and persisted for the session (V1 kept it in session
        ...
        <div className="pl-6">
          {groups.map((group, index) => (
            <IncomingBacklinkGroup
              ...
              // carry its collapsed state from one note's section to the other's.
              key={`${path}:${group.path}`}
              source={group}
              first={index === 0}
              expanded
