 ...
 * dividers between groups. The header toggle collapses the entire list of
 * sources and linking lines, and the choice persists for the session across
 * all notes. Ambient and always-available — the associative recall
        ...
        <div className="mt-5 pl-5">
          {groups.map((group, index) => (
            <BacklinkSourceGroup
              ...
              // and a source shared by two notes must not carry its collapsed
              // state from one note's panel to the other's.
              key={`${path}:${group.path}`}
              source={group}
              first={index === 0}
              expanded
